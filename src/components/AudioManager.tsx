...
export function AudioManager(props: { transcriber: Transcriber }) {
    ...
    const onRecord = async (blob: Blob, arrayBuffer: ArrayBuffer) => {
        const audioCTX = new AudioContext({
            sampleRate: Constants.SAMPLING_RATE,
        });
...
function RecordTile(props: {
    ...
    const { startRecording, stopRecording, recording } = useWhisper({
        ...
        onRecord: async function onRecord(blob, arrayBuffer) {
            if (blob && arrayBuffer) {
                props.onRecord(blob, arrayBuffer);
            }
        },
        // do not make use of auto transcription using openai whisper
        onTranscribe: async function onTranscribe(blob) {
            return { text: "Not used", blob };
        },
    });
    ...
    return (
        <>
            <Tile
                icon={recording ? <StopCircleIcon /> : <MicrophoneIcon />}
